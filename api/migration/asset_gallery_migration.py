...
def _check_table_exists() -> bool:
    ...
    result = db_instance.execute_query_result(f"""
        SELECT EXISTS (
            ...
            WHERE table_schema = 'public' AND table_name = 'asset_galleries'
        )
    """)
...
def _drop_table():
    logging.info(f"..drop asset_galleries if exists")
    db_instance.execute_query(f"""
            DROP TABLE IF EXISTS asset_galleries CASCADE;
            """)

# create asset_galleries table
def _create_table():
    db_instance.execute_query(f"""
...
def init():
    # Step 1.a: DROP Table - asset_galleries
    # _drop_table()
    # Step 1.b: Check if asset_galleries table exists
